...
reponame="docuseal"
docuseal_version="1.5.8"
# Create a new empty container image
container=$(buildah from scratch)

# Reuse existing nodebuilder-docuseal container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-docuseal; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-docuseal -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:15.5-alpine3.19 docker.io/docuseal/docuseal:${docuseal_version}" \
    "${container}"
# Commit the image
